...
window.onload = function() {
    const img = document.getElementById('formatrice');
    const canvas = document.getElementById('canvas');
    const ctx = canvas.getContext('2d');

    // Assurez-vous que l'image est chargée avant de définir la taille du canvas
    img.onload = function() {
        canvas.width = img.width;
        canvas.height = img.height;

        // Centrez le texte en arc
        ...
        for (let i = 0; i < text.length; i++) {
            const angle = (i - (text.length / 2)) * angleStep - (Math.PI / 2);
            ctx.save();
